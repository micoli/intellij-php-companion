...
class FileListenerTest : BasePlatformTestCase() {
    private var fileListener: FileListener<String?>? = null
    private var handledIds: MutableList<String?>? = null
    private var handledFiles: MutableList<VirtualFile?>? = null
    private var patterns: MutableMap<String?, MutableList<PathMatcher?>?>? = null

    @Throws(Exception::class)
    override fun setUp() {
        super.setUp()
        handledIds = ArrayList<String?>()
        handledFiles = ArrayList<VirtualFile?>()

        fileListener =
          FileListener<String?> { id: String?, file: VirtualFile? ->
              handledIds!!.add(id)
    ...
    fun testSetPatternsEnablesListener() {
        initializeListenerAndTriggerFileEvent(
          Map.of<String?, MutableList<PathMatcher?>?>("php", mutableListOf<PathMatcher?>(FileSystems.getDefault().getPathMatcher("glob:**/*.php"))),
    ...
    fun testFileEventHandlingWhenDisabled() {
        ...
        fileListener!!.vfsListener.after(mutableListOf(VFileContentChangeEvent(null, testFile, 0L, 0L)))
    ...
    fun testFileEventHandlingWithMatchingPattern() {
        ...
        initializeListenerAndTriggerFileEvent(
          Map.of<String?, MutableList<PathMatcher?>?>("php-files", mutableListOf<PathMatcher?>(FileSystems.getDefault().getPathMatcher("glob:**/*.php"))),
          listOf(testFile) as MutableList<VirtualFile?>,
        )

        TestCase.assertEquals("One ID should have been processed", 1, handledIds!!.size)
        TestCase.assertEquals("The correct ID should have been processed", "php-files", handledIds!![0])
    ...
    fun testFileEventHandlingWithNonMatchingPattern() {
        initializeListenerAndTriggerFileEvent(
          Map.of<String?, MutableList<PathMatcher?>?>("php-files", mutableListOf<PathMatcher?>(FileSystems.getDefault().getPathMatcher("glob:**/*.php"))),
          listOf<VirtualFile?>(myFixture.createFile("test.js", "console.log('test');")) as MutableList<VirtualFile?>,
        )
    ...
    fun testMultiplePatternsAndIds() {
        ...
        initializeListenerAndTriggerFileEvent(
          Map.of<String?, MutableList<PathMatcher?>?>(
            "php-files",
            mutableListOf<PathMatcher?>(FileSystems.getDefault().getPathMatcher("glob:**/*.php")),
            "js-files",
    ...
    fun testDirectoryEventsAreIgnored() {
        initializeListenerAndTriggerFileEvent(
          Map.of<String?, MutableList<PathMatcher?>?>("all-files", mutableListOf<PathMatcher?>(FileSystems.getDefault().getPathMatcher("glob:**/*"))),
          listOf<VirtualFile?>(myFixture.tempDirFixture.findOrCreateDir("testDir")) as MutableList<VirtualFile?>,
        )
    ...
    private fun initializeListenerAndTriggerFileEvent(patterns: MutableMap<String?, MutableList<PathMatcher?>?>?, testFiles: MutableList<VirtualFile?>) {
        this.patterns = patterns
        this.fileListener!!.setPatterns(patterns)

        val events: MutableList<VFileEvent> =
          testFiles.stream().map { testFile: VirtualFile? -> VFileContentChangeEvent(null, testFile!!, 0L, 0L) }.collect(Collectors.toUnmodifiableList())
