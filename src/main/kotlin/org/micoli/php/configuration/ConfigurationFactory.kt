...
class ConfigurationFactory {
    @JvmField var acceptableConfigurationFilesGlob: String = "glob:**/.php-companion{,.local}.{json,yaml}"

    @JvmField
    val acceptableConfigurationFiles: MutableList<String> = mutableListOf(".php-companion.json", ".php-companion.yaml", ".php-companion.local.json", ".php-companion.local.yaml")
    ...
    fun loadConfiguration(projectPath: String?, previousLatestFileTimestampUpdate: Long, force: Boolean): LoadedConfiguration? {
        val files = acceptableConfigurationFiles.stream().filter { configurationFile: String -> File(projectPath, configurationFile).exists() }.toList()
        if (files.isEmpty()) {
            throw NoConfigurationFileException("No .php-companion(.local).(json|yaml) configuration file(s) found.", 0L)
        }
        ...
        val propertiesHandler = IgnoredPropertiesHandler(listOf(Configuration::class.java) as MutableList<Class<*>>)
        ...
        try {
            ...
            if (!unknownProperties.isEmpty()) {
                throw ConfigurationException("Unrecognized Property: " + unknownProperties.joinToString(", "), latestFileUpdateTimestamp, "", stringContent)
            }
            loadedConfiguration.ignoredProperties = propertiesHandler.getIgnoredProperties()
            return loadedConfiguration
        } catch (configurationException: ConfigurationException) {
            throw ConfigurationException(configurationException.message, latestFileUpdateTimestamp, configurationException.descriptorString, configurationException.originalContent)
        } catch (mappingException: JsonMappingException) {
            throw ConfigurationException(
              (JsonExceptionMapper.getExceptionName(mappingException) + ": " + getReadablePathReference(mappingException)),
              latestFileUpdateTimestamp,
              mappingException.message,
              stringContent,
            )
        } catch (exception: Exception) {
            throw ConfigurationException(exception.message, latestFileUpdateTimestamp, exception.javaClass.descriptorString(), stringContent)
        }
    }
    ...
    @Throws(ConfigurationException::class, IOException::class, JsonObjectExtensionConflictException::class)
    private fun loadConfigurationFiles(projectPath: String?, files: MutableList<String>): String {
        ...
        for (file in files) {
            try {
                ...
                GsonTools.extendJsonObject(mergedJson, ConflictStrategy.PREFER_SECOND_OBJ, asJsonObject)
    ...
    private fun getConfigurationSource(file: String, fullPathFile: File): String {
        ...
        try {
            ...
            return GsonBuilder().setPrettyPrinting().create().toJson(load, LinkedHashMap::class.java)
        } catch (e: ScannerException) {
            throw ConfigurationException(e.problem + "\n" + e.problemMark.toString(), null, e.message, file)
        } catch (e: FileNotFoundException) {
            throw ConfigurationException(e.message, null, e.message, file)
        }
    }

    private fun getLatestFileTimestampUpdate(projectPath: String?, files: MutableList<String>): Long {
        var max = 0L
        for (file in files) {
